...
/* Function to parse the URI and extract hostname, port, and path */
int parse_uri(char *uri, char *hostname, char *port, char *path) 
{
    // Check if the URI is valid (starts with "http://")
    if (strncasecmp(uri, "http://", 7) != 0) {
        fprintf(stderr, "Invalid URI: Only 'http://' is supported.\n");
        return -1;
    }

    // Set pointer to the beginning of the hostname section in the URI
    char *hostbegin = uri + 7;  // Skip over "http://"
    ...
    if (pathbegin) {
        // There is a path in the URI
        strcpy(path, pathbegin);
        *pathbegin = '\0'; // Null-terminate hostname section
    } else {
        // No specific path, default to "/"
        strcpy(path, "/");
    }

    // Check if a port is specified (e.g., hostname:port)
    char *portbegin = strchr(hostbegin, ':');
    if (portbegin) {
        *portbegin = '\0'; // Null-terminate the hostname section
        strcpy(hostname, hostbegin);
        *portbegin = ':';  // Restore the colon for logging/debugging purposes
        portbegin++;
        char *portend = strchr(portbegin, '/');
        if (portend) {
            *portend = '\0'; // Null-terminate the port section
        }
